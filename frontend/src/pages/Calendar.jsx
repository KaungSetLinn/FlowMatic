...
import { faListUl, faPlayCircle, faCheckCircle, faExclamationCircle } from "@fortawesome/free-solid-svg-icons";
...
const Calendar = ({tasks}) => {
  ...
  const taskEvents = tasks.map(task => {
    ...
    const start = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(
      date.getDate()
    ).padStart(2, "0")}`;
  ...
  const sortFunctions = {
    ...
    status: (a, b) =>
      ({ active: 1, completed: 2 }[a.status] - { active: 1, completed: 2 }[b.status]),
  };
  ...
  const handleSave = (evt) => {
    ...
    saveEvents(
      modal.isNew ? [...events, newEvent] : events.map((e) => (e.id === newEvent.id ? newEvent : e)),
  ...
  const closeModal = () => {
    ...
    setTimeout(() => setModalAnimating(false), 750);
  };
  ...
  const updateEvent = (field, value) => setModal((p) => ({ ...p, event: { ...p.event, [field]: value } }));
      ...
      <div className="w-64 bg-white rounded-xl shadow-md p-4 flex-shrink-0">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {filters.map((f) => {
            ...
            const colorClasses =
              {
                blue: isActive ? "bg-blue-600 text-white" : "bg-white border border-blue-300 text-blue-600 hover:bg-blue-50",
                yellow: isActive ? "bg-yellow-500 text-white" : "bg-white border border-yellow-300 text-yellow-600 hover:bg-yellow-50",
                green: isActive ? "bg-green-600 text-white" : "bg-white border border-green-300 text-green-600 hover:bg-green-50",
                red: isActive ? "bg-red-600 text-white" : "bg-white border border-red-300 text-red-600 hover:bg-red-50",
              }[f.color];
            return (
              <button key={f.type} onClick={() => setFilter(f.type)} className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${colorClasses}`}>
        ...
        <div className="space-y-2 max-h-[80vh] overflow-y-auto">
          {events
            .filter((e) =>
              filter === "all" ? true : filter === "high" ? new Date(e.start) - new Date() <= 3 * 24 * 60 * 60 * 1000 : e.status === filter
            )
  ...
  textColor: "#fff",
          ...
          dayCellContent={(arg) => {
            ...
            return <div className="text-sm font-medium" style={{ color: d === 0 ? "#ef4444" : d === 6 ? "#3b82f6" : "#374151" }}>{arg.date.getDate()}</div>;
          }}
          eventContent={(arg) => (
            <div className={`whitespace-normal text-sm font-semibold ${arg.event.extendedProps?.status === "completed" ? "line-through" : ""}`} style={{ backgroundColor: arg.event.backgroundColor, color: arg.event.textColor, borderRadius: "4px", padding: "1px 3px" }}>
              {arg.event.title}
            </div>
          )}
          select={(info) => {
            const allDay = info.allDay;
            const start = allDay ? formatDateJP(info.start) : formatDateJP(info.start) + "T09:00";
            const end = allDay ? formatDateJP(info.end) : formatDateJP(info.end) + "T10:00";
            openModal({ id: crypto.randomUUID(), title: "", start, end, color: "#3b82f6", allDay, priority: "medium", status: "active" }, true);
          }}
          height="auto"
          contentHeight="auto"
          eventDrop={(info) => {
            const updated = events.map((e) => (e.id === info.event.id ? { ...e, start: info.event.startStr, end: info.event.endStr } : e));
      ...
      <div className="fixed top-5 right-5 space-y-2 z-[2000]">
        {notifications.map((n) => (
          <div key={n.id} className="bg-blue-600 text-white px-4 py-2 rounded-md shadow-lg animate-slide-in">
            {n.text}
          </div>
        ))}
      </div>
                  ...
                  onChange={(e) => {
                    ...
                    setModal((p) => ({
                      ...
                      event: {
                        ...p.event,
                        allDay: isAllDay,
                        start: isAllDay ? p.event.start.split("T")[0] : formatDateJP(new Date(p.event.start)) + "T09:00",
                        end: isAllDay ? p.event.end.split("T")[0] : formatDateJP(new Date(p.event.end)) + "T10:00",
                      },
                    }));
                  }}
                />
