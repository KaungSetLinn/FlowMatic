...
class ChatRoomMessageListCreateView(APIView):
	...
	def get(self, request, chatroom_id: str) -> Response:
		...
		return Response({'messages': serializer.data, 'page': page, 'per_page': per_page})

	def post(self, request, chatroom_id: str) -> Response:
		chatroom = self._get_chatroom(chatroom_id)
		serializer = MessageCreateSerializer(data=request.data, context={'chatroom': chatroom, 'request': request})
		serializer.is_valid(raise_exception=True)
